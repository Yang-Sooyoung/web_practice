...
public class Java100_oop_Polymorphism2 {
	public static void main(String[] args) {
		...
		// [1] : 객체 생성
		Student10 s3 = new Student10();	// 지극히 정상 --> 에러날 이유가 하나도 없다.
		
		// [2] : 객체 생성 --> 타입은 부모 타입으로 생성
		Person10 s4 = new Student10();  	// 이것도 정상 --> 하위(자식) 클래스로 객체를 만들면서 타입은 상위(부모) 타입을 쓰는 것이 가능 --> 다형성
		
		// [3] : 객체 생성
		Person10 aaaaa = new Person10();	// 지극히 정상
		
		// [4] : 객체 생성 --> 상위(부모) 클래스로 객체를 생성하면서 타입은 하위(자식) 타입을 쓰는 경우 --> Err
		// Student10 bbb = new Person10();	// Err
